        ...
        <nav className={` bg-transparent w-full fixed top-0 z-50 ${currentNav ? "h-fit" : "h-20"} `} onMouseLeave={() => setCurrentNav(null)}>
            <div className="w-full bg-white">
                <div className={`w-full bg-white max-w-7xl  mx-auto h-20 my-auto flex flex-row justify-between py-4 items-center`}>
                    ...
                    <div className="hidden lg:flex items-center gap-4 hover:cursor-pointer mr-4">
                        <button 
                    ...
                    {/* Mobile Language Toggle & Menu Button - Shows up to LG screens */}
                    <div className="lg:hidden mr-4 my-auto flex items-center gap-3">
                        <button 
                            onClick={() => setLanguage(language === 'en' ? 'fr' : 'en')}
                            className="flex items-center gap-2 px-3 py-2 rounded-lg  hover:bg-gray-200 transition-colors duration-300 font-dm-sans font-medium text-primary"
                        >
